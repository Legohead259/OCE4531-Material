...
\emph{If it is not clear where your challenge-specific code is, it may not be counted.}
    ...
    First, you will implement a Mahony filter to convert the IMU readings to orientation values.
    Do not worry, you will not need to research the theory or algorithm behind this filter, there is a \href{https://github.com/PaulStoffregen/MahonyAHRS}{handy library} that is easy to use and has some example code.
    
    Second, you will use a Kalman filter to estimate the velocity and displacement values.
    Realistically, this would be done with complex matrix math and with a single 9-dimensional Kalman filter object, but for sake of ease, we will just use six 1-dimensional Kalman filters; three for velocity X/Y/Z, and three for displacement X/Y/Z.
